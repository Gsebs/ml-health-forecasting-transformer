...
"""
Evaluation Script for Transformer RUL Prediction

This script evaluates a trained TransformerBaseline model on the test dataset.
Key features:
1. Robust Data Alignment: Ensures predictions are compared against the correct ground truth engines.
2. Dynamic Feature Loading: Uses the same input features as training (from input_features.json).
3. Metric Calculation: Computes Root Mean Squared Error (RMSE) for Remaining Useful Life (RUL).
4. Visualization: Generates a plot of Predicted vs Actual RUL.
...
def plot_results(y_true, y_pred, rmse):
    ...
    plt.title(f'Transformer RUL Prediction (RMSE: {rmse:.2f})')
...
def main():
    ...
    try:
        ...
        model = TransformerBaseline(input_dim=len(training_features))
